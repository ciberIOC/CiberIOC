...
En el següent document explica quines son les passes per realitzar un EAC i com poder utilitzar els arxius d'autocorrecció per a màquines Linux.
...
El redactor del EAC penjarà un fil al fòrum de l'aula de coordinació el títol "MP XX EAC X Semestre X XX-XX" (Exemple: MP01 EAC 1 Semestre 1 24-25". En aquest post es posarà la següent informació:
...
En aquest pas el redactor del EAC crearà l'enunciat sobre el que treballarà l'alumnat. L'enunciat es publicarà en el mateix format que tenim la documentació de l'aula i serà feina del coordiandor el donar el format i publicar-ho al Moodle.
...
El coordinador realitzarà l'activitat, i si tot és correcte, publicarà l'enunciat al Moodle i deixarà una còpia amb el format nou  al fòrum de Coordinació.

En el post de l'enunciat, el redactor del EAC també haurà de publicar els mecanismes d'avaluació que s'utilitzarà, si es vol que es publiqui la rúbrica d'avaluació, si vol crear un debat "fòrum" intern dins l'activitat. Caldrà detallar tots aquells aspectes per tal que el coordinador pugui realitzar penjar l'activitat al curs Moodle.

## Pas 3: Realització EAC

Durant la realització del EAC cal anar realitzant el suport necessari a l’alumnat per part del corrector. A nivell global, podem estandarditzar els següents missatges als fòrum:
* Missatge inicial, el dia que comença el EAC. 
...
D’altra banda, cal informar si necessiteu màquines penjades a l’IsardVDI a nivell de servidor. Si necessiteu que jo pengi o despengi alguna màquina a l’isard durant uns dies determinat ho podeu marcar al calendari google i m’afegiu ‘jbella@ioc.cat’. D’aquesta manera, m’enrecordaré d’anar penjant i tencant els servidors. Les plantilles i els desplegaments els podeu realitzar vosaltres. Recordeu que per tal que la vegi l’alumnat s’ha de compartir a través del grup del mòdul: FPINF_CIB_MXX.

## Pas 4: Solució

Tots els EACs hauran de tenir dues retroaccions:
1) Retroacció en text, realitzada pel redactor del EAC, és la resposta "típica" d'un EAC que hauria de rebre l'alumnat. Pot ser per exemple, si és un qüestionari que es mostri la resposta correcte, o si son un conjunt de passos a realitzar, doncs mostrar els passos a fer. Pot tenir format PDF, o es pot mostrar dins l'exercici mateix.
2) Retroacció en vídeo, realitzada pel corrector, aquesta retroacció hauria de mostrar com arribar a la solució, i es realitza sobretot, per tal l'alumnat pugui entendre com s'hi arriba. Lo important dels vídeos és que l'alumnat pugui entendre l'EAC.
...
Per acabar, el corrector del EAC avisarà a través del fòrum quan les qualificacions estiguin penjades. El coordinador farà visibles els resultats. Si no s'informa del contrari, el coordinador publicarà els resultats a les 12:00h del migdia, d'aquesta manera, el corrector podrà programar un missatge per la mateixa hora.

En el missatge final, el corrector haurà de mostrar les principals dades estadístiques. (En procés de realització, cal crear una plantilla sobre la que importar els arxius csv de Moodle i que cregui un dashboard que es pugui publicar al Moodle. Podem començar amb un simple gràfic de quartils).

## Pas 6: Retroacció EAC per part del corrector/redactor/coordinador

Una vegada obtinguts els resultats, corrector i/o redactor, en el fil del fòrum del EAC, publicaran una valoració sobre si cal canviar algun aspecte del EAC de cara al any vinent. Es tracta d'avaluar el EAC 

Per acabar, coordinador fa valoració final del EAC tenint en compte els resultats del EAC i els comentaris de corrector/redactor.

Es dona per finalitzat el EAC.
...
Aquest document té per objectiu que el conjunt de professors col·laboradors mantinguin una mateixa estructura organitzativa per tal de facilitar la feina a l’alumnat i donar una senyal de continuat en els materials entre mòduls.
...
El document estableix inicialment el marc pedagògic sobre el que treballarem, la estructura bàsica d’un EAC, els recursos que teniu disponibles per a realitzar els exercicis, els mecanismes d’avaluació, i per últim, els diversos tipus d’EACs que es poden realitzar.

## Marc pedagògic

A nivell pedagògic, un EAC pot entendre's de tres formes diferents: com un repàs d’aquells continguts que s’han donat en el marc teòric del mòdul, com una ‘posada en pràctica’ del que s’ha après a nivell teòric, o com un ‘més a més’ del que s’ha fet a l’aula per tal d’actualitzar el mòdul amb continguts més moderns o més pràctics.

Al llarg del curs, és molt probable que el professorat utilitzi diversos marcs pedagògics, una bona combinació d’aquests acabarà oferint una sensació d'aprenentatge òptima per part de l’alumnat. A l’hora, també cal tenir en compte el propi mòdul, n’hi ha que son una seqüència d’aprenentatges constructius, mentre que n’hi ha d’altres que els aprenentatges de les unitats son independents entre ells.
...
En la elaboració del EAC hi ha tres rols:
* **Coordinador**: És el coordiandor del mòdul, s'encarrega sobretot de la maquetació i de preparar-ne la seva publicació.
* **Redactor**: És qui prepara tot el EAC, totes les màquines, i el sistema d'avaluació.
...
Dins el llibre, inclourem tota la informació de l’EAC de l’estructura bàsica excepte l’enunciat. Dins el llibre, a més, també hi haurà els advertiments de còpia o males praxis generals.
...
IsardVDI ofereix la opció de crear plantilles i/o crear desplegaments. Una plantilla és una màquina que pot recrear qualsevol alumne i fer-se-la seva. Un desplegament és una màquina configurada i creada pel professor, que es desplega en el conjunt d'alumnes que nosaltres volem, i que l'alumnat no pot formatejar.

Els desplegaments els vam provar inicialment, ofereixen la possibilitat de replicar una plantilla en cadascun dels alumnes i poder-ne fer un seguiment. Però no els hi dona la opció de reiniciar de forma autònoma les seves màquines i ho vam anar descartant.
...
Actualment estem treballant la possibilitat de tenir servidors oberts a l’exterior. La idea és crear un pont entre una IP pública de l’IOC i una xarxa pública de l’IsardVDI. D’aquesta manera, poderm incloure els servidors que vulguem.

D’entrada, volem crear un servidor CTF per poder realitzar activitats per l’alumnat.
...
* En els qüestionaris tipus test teòrics: cal buscar la manera per tal que l'alumnat es pugui autoesmenar.
...
Son els qüestionaris que podem realtizar i que obliguen a l’alumnat a trobar la resposta a través de la consulta dels materials.
...
Son la solució que més s’adapta a la línia pedagògica del curs ja que ofereix el mateix entorn a l’alumnat, son fàcilment escalables i ofereixen possibilitats d’autocorrecció.
...
* L’alumnat ha de ser capaç d’executar un petit programa en Python que evalúa el seu exercici i li mostra on estan els errors.
* L’alumnat ha de ser capaç de veure el codi d’aquest programa i li facilita entendre on es pot haver equivocat.
* És una bona praxis obrir un qüestionari al Moodle recollint el nombre d'encerts té un l’alumnat. Caldrà alhora, habilitar alguna captura de pantalla que acabi de comprovar l’autoria i èxit de l’exercici, per exemple, una captura PCAP.
* Sempre caldrà realitzar un vídeo explicant com es feia l’exercici i que serveixi a l’alumnat per entendre l’exercici. Aquest vídeo és per quan ja ha acabat l’exercici i us pot servir per fer preguntes després a les PAF.


#### Exercicis pràctics en local

Poden ser una solució òptima per quan l’IsardVDI impedeix realitzar correctament la pràctica. 
...
* L’alumnat ha de ser capaç d’entendre la evolució de la seva pràctica i conèixer si prota el camí correcte.
...
L'objectius dels col·laboradors dels cursos d'especialització és que es converteixin en docents experts i aportin recursos, suport i aprenentatges a l'alumnat. Actualment, el col·laborador està amb la sensació que la major part del seu temps es dedica a la correcció. A tall d'exemple, comprovar 50 configuracions de diversos tipus i diverses eines que es puguin desplegar en un entorn d'aprenentatge, poden suposar moltes hores d'esforç, que no queda clar que hi hagi un retorn avaluatiu vers a l'alumnnat. Per contra, buscar la forma en que l'alumnat es pugui anar autoavaluant, pugui millorar el seu aprenentatge a través d'aquesta avaluació, i el docent eviti hores de correcció, doncs s'entèn que suposa un salt de qualitat important en favor, no només de l'alumnat que és evident, sinó també de la motivació del professorat.
...
D'entrada, la vostra pràctica/enunciat no hauria de variar si fem autocorrecció o no. Anem a observar l'esquema de la vostra possible pràctica per entendre totes les parts:
...
En aquest moment ja tenim clar quins seran els punts de control, l'enunciat i el sistema sobre el que treballarem.
...
Hem d'assegurar-nos que el hostname és isard, d'aquesta manera se li demanarà que canvii el hostname quan arrenqui la màquina de comprovació:
...
Cal canviar el nom de l'arxiu i treure el sufit *.original*.
...
Una vegada ja ho tingueu tot preparat, ja podeu crear la plantilla sobre la que es basarà el vostre desplegament. Abans de crear-la, tingueu en compte:

**NOM PLANTILLA**, el nom de la plantilla serà: "MPXX-EACXX-CURSXX:XX-CLIENT_AVAL_TEMPLATE"

**MACHINE-ID i HOSTNAME**, comproveu que estan correctes abans de crear la plantilla i tencar la màquina.

**GRUP PLANTILLA**, d'entrada, posareu la plantilla en el grup de professors col·laboradors: FPINF_CIB_COLAB.

**Xarxes**, comproveu que teniu les xarxes ben configurades i que no es solaparan.
...
Tota la sortida es guardarà en dos arxius que vosaltres definireu. En un arxiu obtindreu la última avaluació, i per l'altra, obtindreu un arxiu de text amb la sortida de MongoDB amb tots els intents.

## Pas 5: Gaurdar els resultats a l'aula de coordinació i reptes de futur
